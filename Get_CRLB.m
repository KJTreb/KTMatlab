function [CRLB_I, CRLB_Gd, CRLB_PMMA] = Get_CRLB(spectrum_Low, spectrum_High, PCD_threshold)
...
A_1 = 2.5*rho_1; %iodine solution thickness in cm times
A_2 = 2.5*rho_2; %Gd solution thickness in cm times
...
for PCD_thresh = PCD_threshold
    ...
    bin_spectrum_1 = bin_spectrum_Low_LE + bin_spectrum_High_LE;
    bin_spectrum_2 = bin_spectrum_Low_HE;
    bin_spectrum_3 = bin_spectrum_High_HE;
    
    M = zeros(3,3);
    M(1,1)=sum(mu_1.*bin_spectrum_1)/sum(bin_spectrum_1);
    M(1,2)=sum(mu_1.*bin_spectrum_2)/sum(bin_spectrum_2);
    M(1,3)=sum(mu_1.*bin_spectrum_3)/sum(bin_spectrum_3);
    M(2,1)=sum(mu_2.*bin_spectrum_1)/sum(bin_spectrum_1);
    M(2,2)=sum(mu_2.*bin_spectrum_2)/sum(bin_spectrum_2);
    M(2,3)=sum(mu_2.*bin_spectrum_3)/sum(bin_spectrum_3);
    M(3,1)=sum(mu_3.*bin_spectrum_1)/sum(bin_spectrum_1);
    M(3,2)=sum(mu_3.*bin_spectrum_2)/sum(bin_spectrum_2);
    M(3,3)=sum(mu_3.*bin_spectrum_3)/sum(bin_spectrum_3);
    ...
    N_1=N_High*sum(bin_spectrum_High_LE) + N_Low*sum(bin_spectrum_Low_LE);
    N_2=N_Low*sum(bin_spectrum_Low_HE);
    N_3=N_High*sum(bin_spectrum_High_HE);
    
    
    cov_matrix=inv(M')*[1/N_1, 0, 0;0 1/N_2, 0; 0, 0, 1/N_3]*inv(M);
