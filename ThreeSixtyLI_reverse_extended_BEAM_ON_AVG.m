            ...
            if j==1
                if (4.9959<z && z<5.2387) %inside rest zone and at angle 0
                    ...
                    for k=1:length(projIndexLow)
                        sgmHelical(j,:,i) = sgmHelical(j,:,i)+sgm(projIndexLow(k),:)./length(projIndexLow);
                    end
...
for i=1:67
    ...
    imgtemp = MgRingCorrection(imgtemp, -1000, 1000, 100, 15, 70);
    img(:,:,i) = imgtemp;
end

MgSaveRawFile('img/ReverseHelical_tube_on/head_lowmA_48point9_img_bh_ring_1_512-512-67.raw', img);